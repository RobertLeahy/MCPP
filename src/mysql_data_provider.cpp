	...
	void MySQLDataProvider::SaveColumn (Int32 x, Int32 z, SByte dimension, ColumnSaveBegin callback_begin, ColumnSaveEnd callback_end) {
		...
		pool.Enqueue([=] () {
			...
			try {
				...
				//	Attempt UPDATE
				if (
					(mysql_stmt_bind_param(column_update_stmt,param)!=0) ||
					(mysql_stmt_execute(column_update_stmt)!=0)
				) throw std::runtime_error(mysql_stmt_error(column_update_stmt));
				
				++requests;
				
				//	Did we succeed in saving?
				if (mysql_stmt_affected_rows(column_update_stmt)==0) {
				
					//	NO
					
					//	Regenerate the INSERT statement
					//	if necessary
					prepare_stmt(column_insert_stmt,column_insert_query);
					
					//	Attempt INSERT
					if (
						(mysql_stmt_bind_param(column_insert_stmt,param)!=0) ||
						(mysql_stmt_execute(column_insert_stmt)!=0)
					) throw std::runtime_error(mysql_stmt_error(column_insert_stmt));
