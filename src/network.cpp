...
namespace MCPP {
	...
	SmartPointer<Connection> Connection::disconnect () {
		...
		if (is_shutdown) {
			...
			return SmartPointer<Connection>();
		...
		//	Now we go into the handler to retrieve
		//	ourselves
		handler.connections_lock.Acquire();
		
		//	Find ourselves
		auto self=handler.connections[this];
		
		//	We're done with the handler and
		//	ourself
		handler.connections_lock.Release();
		lock.Release();
		
		//	We need to send the handler a
		//	message which instructs it to
		//	disconnect us.
		disconnect_request=SmartPointer<OverlappedData>::Make(NetworkCommand::Disconnect);
		
		if (!PostQueuedCompletionStatus(
			iocp,
			0,
			reinterpret_cast<ULONG_PTR>(this),
			static_cast<OverlappedData *>(disconnect_request)
	...
	)	:	disconnect_callback(handler.disconnect_callback),
			socket(socket),
			endpoint(endpoint),
			handler(handler),
			iocp(iocp),
	...
	inline void ConnectionHandler::kill (Connection * conn) {
	...
	void ConnectionHandler::worker_func () noexcept {
		...
		try {
			...
			for (;;) {
				...
				//	If we're no longer responsible for this
				//	connection, pass
						...
						try {
							...
							pool.Enqueue(
								[this] (SmartPointer<Connection> conn) {
									...
									//	Only proceed if the socket
									//	isn't marked for death
									conn->lock.Acquire();
									if (conn->is_shutdown) {
									...
									if (!PostQueuedCompletionStatus(
										...
										try {	kill(static_cast<Connection *>(conn));	} catch (...) {	}
									...
									//	Connection now has a queued receive
	...
	ConnectionHandler::~ConnectionHandler () noexcept {
		...
		for (auto & pair : connections) pair.second->Disconnect();
